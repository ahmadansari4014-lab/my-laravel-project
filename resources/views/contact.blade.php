@extends('layouts.app')

@section('content')
<div class="relative pb-20">

    <!-- Cinematic Hero Section -->
    <div class="relative w-full py-32 md:py-48 flex items-center justify-center rounded-[3rem] overflow-hidden mb-24 shadow-2xl mx-6 group">
        <div class="absolute inset-0">
            <img src="https://images.unsplash.com/photo-1596524430615-b4661858a36c?q=80&w=2000&auto=format&fit=crop" class="w-full h-full object-cover transition-transform duration-[20s] group-hover:scale-110 grayscale" alt="">
            <div class="absolute inset-0 bg-gradient-to-br from-slate-900/95 via-blue-900/80 to-slate-900/95 mix-blend-multiply"></div>
        </div>
        
        <div class="relative z-10 w-full max-w-5xl mx-auto px-6 text-center">
            <h1 class="text-5xl md:text-7xl font-extrabold text-white mb-6 tracking-tight drop-shadow-lg">
                Let's Build <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">Something Great</span>
            </h1>
            <p class="text-xl text-slate-300 max-w-3xl mx-auto font-light leading-relaxed">
                Ready to delegate, optimize, and dominate? Reach out to our architecture team today.
            </p>
        </div>
    </div>

    <!-- Contact Container -->
    <div class="max-w-6xl mx-auto px-6">
        <div class="bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col lg:flex-row">
            
            <!-- Form Section -->
            <div class="w-full lg:w-3/5 p-10 md:p-16">
                <!-- NEW: Updated Heading & Subtext -->
                <h2 class="text-3xl font-extrabold text-slate-900 mb-2">Book Your Free Operations Audit</h2>
                <p class="text-slate-600 mb-10">Tell us about your biggest operational challenges. An account manager will reach out within 24 hours to schedule your 30-minute review.</p>
                
                <form action="{{ route('contact.send') }}" method="POST" class="space-y-6">
                    @csrf
                    
                    <!-- Success Message Alert -->
                    @if(session('success'))
                        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4">
                            <span class="block sm:inline">{{ session('success') }}</span>
                        </div>
                    @endif

                    <!-- Error Message Alert -->
                    @if(session('error'))
                        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4">
                            <span class="block sm:inline font-bold">Failed to send!</span>
                            <span class="block sm:inline text-sm">{{ session('error') }}</span>
                        </div>
                    @endif

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Full Name</label>
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-5 py-4 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition">
                            @error('name') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Work Email</label>
                            <input type="email" name="email" value="{{ old('email') }}" required class="w-full px-5 py-4 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition">
                            @error('email') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Service Interest</label>
                        <select name="service" required class="w-full px-5 py-4 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition bg-white">
                            <option value="Virtual Assistance" {{ old('service') == 'Virtual Assistance' ? 'selected' : '' }}>Virtual Assistance</option>
                            <option value="Digital Growth & SEO" {{ old('service') == 'Digital Growth & SEO' ? 'selected' : '' }}>Digital Growth & SEO</option>
                            <option value="Financial Bookkeeping" {{ old('service') == 'Financial Bookkeeping' ? 'selected' : '' }}>Financial Bookkeeping</option>
                            <option value="Web Architecture" {{ old('service') == 'Web Architecture' ? 'selected' : '' }}>Web Architecture</option>
                        </select>
                        @error('service') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Project Details</label>
                        <textarea name="message" rows="4" required class="w-full px-5 py-4 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition">{{ old('message') }}</textarea>
                        @error('message') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    
                    <!-- NEW: Updated Button Text -->
                    <button type="submit" class="w-full bg-slate-900 text-white font-bold py-5 rounded-xl hover:bg-blue-600 transition shadow-lg hover:shadow-blue-500/30 transform hover:-translate-y-1">
                        Request Free Audit
                    </button>
                </form>
            </div>

            <!-- Contact Info Section -->
            <div class="w-full lg:w-2/5 bg-slate-50 p-10 md:p-16 border-l border-slate-100 flex flex-col justify-center">
                <div class="space-y-10">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-900 mb-6">Let's Talk</h3>
                        <p class="text-slate-600 mb-8">Whether you have a question about our services or are ready to get started, our team is standing by.</p>
                        
                        <!-- Direct Gmail Link -->
                        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=info@clouddextra.com" target="_blank" class="flex items-center gap-4 text-blue-600 hover:text-blue-800 transition-colors font-bold text-lg mb-6 group">
                            <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-md group-hover:bg-blue-600 group-hover:text-white transition">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                            </div>
                            info@clouddextra.com
                        </a>

                        <!-- Clickable Website -->
                        <a href="https://clouddextra.com" target="_blank" class="flex items-center gap-4 text-blue-600 hover:text-blue-800 transition-colors font-bold text-lg group">
                            <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-md group-hover:bg-blue-600 group-hover:text-white transition">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg>
                            </div>
                            clouddextra.com
                        </a>
                    </div>

                    <div class="border-t border-slate-200 pt-10">
                        <h4 class="font-bold text-slate-900 mb-4">Office Hours</h4>
                        <p class="text-slate-600">Monday – Friday: 9:00 AM – 6:00 PM</p>
                        <p class="text-slate-600">Weekend support available via email.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection